...
exports.up = knex =>
  knex.schema.createTable('restaurant_role_rights', table => {
    ...
    table.integer('restaurant')
      ...
      .onDelete('CASCADE')
      .unsigned();
    table.boolean('allow_add_promotion').notNullable().defaultTo(false);
    table.boolean('allow_alter_promotion').notNullable().defaultTo(false);
    table.boolean('allow_delete_promotion').notNullable().defaultTo(false);
    table.boolean('allow_add_menu').notNullable().defaultTo(false);
    table.boolean('allow_alter_menu').notNullable().defaultTo(false);
    table.boolean('allow_delete_menu').notNullable().defaultTo(false);
    table.boolean('allow_add_menu_item').notNullable().defaultTo(false);
    table.boolean('allow_alter_menu_item').notNullable().defaultTo(false);
    ...
    table.boolean('allow_alter_restaurant_roles').notNullable().defaultTo(false);
  ...
  .then(() => knex.raw('GRANT SELECT ON restaurant_role_rights TO guest'))
  .then(() => knex.raw(`
    ...
    USING (
      ...
      EXISTS (
        ...
        SELECT 1 FROM restaurant_account WHERE
          ...
          (allow_alter_restaurant_roles OR allow_map_roles)
        ORDER BY restaurant_role_rights.restaurant NULLS LAST LIMIT 1
      )
    )
    WITH CHECK (EXISTS(
